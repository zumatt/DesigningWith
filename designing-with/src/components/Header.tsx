  ...
  return (
    <div className="h-full w-full">
      <div className="flex flex-row gap-2">
        ...
        >
          <p className="text-[2.4vh]" style={ {transition: "all 300ms", transform: menuIsOpen ? "rotate(45deg)" : ""}}>+</p>
        </button>
        {menuIsOpen ? <Results /> : null}
        {!menuIsOpen && pathname === "/about" && (
          <p className="w-fit text-[2.4vh] bg-purple rounded-xl px-3">About </p>
        )}
        ...
        {!menuIsOpen && pathname === "/interactive-framework" && (
          <p className="w-fit text-[2.4vh] bg-grey rounded-xl px-3">
            Interactive Framework{" "}
          </p>
        )}
        {!menuIsOpen && pathname === "/didactic-guidelines" && (
          <p className="w-fit text-[2.4vh] bg-green rounded-xl px-3">Didactic Guidelines </p>
        )}
        {!menuIsOpen && pathname === "/resources" && (
          <p className="w-fit text-[2.4vh] bg-orange rounded-xl px-3">Resources </p>
        )}
        {!menuIsOpen && pathname === "/" && (
          <p className="w-fit text-[2.4vh] bg-beige rounded-xl px-3">Let’s talk! </p>
        )}
      </div>
...
const Results = () => (
  <div className="flex flex-col h-4 gap-2 z-10">
    ...
    >
      <p className="text-[2.4vh]">Let’s talk!</p>
    </a>
  </div>
);
